...
select DATEADD(DAY, @days, 
	...
	dateadd(second, datepart(second, b.expTime), dateadd(minute, datepart(minute, b.expTime), dateadd(hour, datepart(hour, b.expTime), cast(dateadd(day, b.expDate, cast(GETDATE() as date)) as datetime))))
from demo.dbo.Deal2 a
left outer join Boards b on a.boardid=b.id
